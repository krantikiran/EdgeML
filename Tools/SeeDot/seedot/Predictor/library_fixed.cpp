...
int32_t fixedTanH(int32_t x){
	...
	int32_t scale_factor = 1<<SCALE_OUT;
    int32_t exp_res;
    int32_t ans;
    if (x < 0)
    {
        exp_res = fixedExp(2*x);
        ans = int64_t((int64_t(exp_res - scale_factor))<<SCALE_OUT)/(int64_t(scale_factor + exp_res));
    }
    else
    {
        exp_res = fixedExp(-2*x);
        ans = int64_t((int64_t(scale_factor-exp_res))<<SCALE_OUT)/(int64_t(scale_factor + exp_res));
    }
    return ans;
}
...
void TanH(MYINT *A, MYINT I, MYINT J, MYINT scale_in, MYINT scale_out, MYINT *B)
{
	...
	for (MYITE i = 0; i < I; i++)
	{
		for (MYITE j = 0; j < J; j++)
		{
			...
			int32_t tanh_res = fixedTanH(A[i * J + j]);
			// tanh_res = (scale_in > scale_out)? (tanh_res / (scale_in/scale_out)) : (tanh_res * (scale_out/scale_in));
			B[i * J + j] = tanh_res;
#else
			MYINT x = A[i * J + j], y;
...
#define INT64_MAX 0x7777777777777777
#define INT_MIN (-INT_MAX-1)
#define __LOG2E__ (1.44269504089 * (1<<SCALE))
#define __LOGE2__ (0.69314718056 * (1<<SCALE))
...
int32_t fixedExp(int32_t x){
	...
	int32_t scale_diff = SCALE_OUT - SCALE;
	N = N+scale_diff;
...
void Exp(MYINT *A, MYINT I, MYINT J, MYINT shrA, MYINT shrB, MYINT *B)
{
	...
	for (MYITE i = 0; i < I; i++)
	{
		for (MYITE j = 0; j < J; j++)
		{
			int32_t exp_res = fixedExp(A[i * J + j]);
			// exp_res = (shrA > shrB)? (exp_res / (shrA/shrB)) : (exp_res * (shrB/shrA)); 
			B[i * J + j] = ((MYINT)exp_res);//(MYINT)(exp(float(A[i * J + j])/float(shrA))* float(shrB));//float(expTableTmp(float(A[i * J + j])*(float(1<<26)/shrA))) * (float(shrB) / float(1<<30));
		}
	}
...
int32_t fixedSigmoid(int32_t x){
    ...
    {
        exp_res = fixedExp(x);
        ans = (int64_t(exp_res) * int64_t(scale_factor))/(scale_factor + exp_res);
    }
    else
    {
        exp_res = fixedExp(-x);
        ans = (int64_t(scale_factor)*int64_t(scale_factor))/(scale_factor + exp_res);
    }
    return ans;
}
...
void Sigmoid(MYINT *A, MYINT I, MYINT J, MYINT div, MYINT add, MYINT sigmoid_limit, MYINT scale_in, MYINT scale_out, MYINT *B)
{
	...
	for (MYITE i = 0; i < I; i++)
	{
		for (MYITE j = 0; j < J; j++)
		{
			...
			int32_t sigmoid_res = fixedSigmoid(A[i * J + j]);
			// sigmoid_res = (scale_in > scale_out)? (sigmoid_res / (scale_in/scale_out)) : (sigmoid_res * (scale_out/scale_in));
			B[i * J + j] = sigmoid_res;//z;
#else
			MYINT x = A[i * J + j];
